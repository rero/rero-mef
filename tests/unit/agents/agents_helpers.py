...
def trans_prep(source, xml_part_to_add):
    ...
    logger = Logger()
    trans = {
        'gnd': Transformation_gnd(marc=records[0], logger=logger,
                                  verbose=True, transform=False),
        'idref': Transformation_idref(marc=records[0], logger=logger,
                                      verbose=True, transform=False),
        'rero': Transformation_rero(marc=records[0], logger=logger,
                                    verbose=True, transform=False)
    }
