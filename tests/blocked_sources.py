...
def block_external_sources(request, monkeypatch):
    ...
    :param monkeypatch: Pytest monkeypatch, used to state the hosts no proxy may carry.
    """
    ...
    # A proxy would carry the request to the service itself, `guard` seeing the address of the proxy and nothing
    # else. The blocked hosts are therefore stated in `NO_PROXY`, which takes them out of whatever proxy the
    # environment sets, for them alone.
    blocked = ",".join(sorted(BLOCKED_HOSTS))
    for name in ("NO_PROXY", "no_proxy"):
        stated = os.environ.get(name)
        monkeypatch.setenv(name, f"{stated},{blocked}" if stated else blocked)
