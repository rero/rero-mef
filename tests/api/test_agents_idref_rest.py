...
def test_view_agents_idref(client, agent_idref_record):
    ...
    url = url_for('api_blueprint.redirect_list_idref')
    res = client.get(url)
    ...
    url = url_for('api_blueprint.redirect_item_idref', pid=pid)
    res = client.get(url)
    assert res.status_code == 308
    res = client.get(url, follow_redirects=True)
    assert res.status_code == 200
    ...
    url = url_for('api_blueprint.redirect_item_idref', pid='WRONG')
    res = client.get(url, follow_redirects=True)
