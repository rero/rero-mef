...
from rero_mef.agents import AgentGndRecord, AgentIdrefRecord, AgentMefRecord, \
    AgentReroRecord, AgentViafRecord
...
def agent_viaf_record(
    app, agent_viaf_data, agent_idref_record, agent_rero_record,
    agent_gnd_record
):
...
def agent_viaf_gnd_redirect_record(
    app, agent_viaf_gnd_redirect_data, agent_idref_record, agent_rero_record,
...
def agent_viaf_idref_redirect_record(
    app, agent_viaf_idref_redirect_data, agent_idref_redirect_record,
    agent_rero_record, agent_gnd_redirect_record
):
    """Create a VIAF record."""
...
@pytest.fixture(scope='module')
def agent_mef_gnd_redirect_record(app, agent_mef_gnd_redirect_data,
                                  agent_viaf_gnd_redirect_record):
    """Create a IdRef record."""
    return create_record(AgentMefRecord, agent_mef_gnd_redirect_data)


@pytest.fixture(scope='module')
def agent_mef_idref_redirect_record(app, agent_mef_idref_redirect_data,
                                    agent_viaf_idref_redirect_record):
    """Create a IdRef record."""
