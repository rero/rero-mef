...
- Types: `feat`, `fix`, `chore`, `docs`, `refactor`, `test`, `style`, `perf`,
  `build`, `ci`, `revert`.
- Scope: the package under `rero_mef/` that the change touches (`agents`,
  `concepts`, `places`, `marctojson`, `monitoring`). Elsewhere, use the area,
  as the history already does — `mef`, `extensions`, `deps`, `docker`, `docs`,
  `dev`. Check with
  ...
  scope.
- `!` before the colon marks a breaking change: `feat(agents)!: ...`.
