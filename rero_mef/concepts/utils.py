...
#: `FRBNF` plus eight digits, or the same number in a BNF ark uri, each with the single check character BNF
#: appends to it. Read with `fullmatch`: a value stating anything more is not one of these numbers.
BNF_NUMBER = re.compile(r"(?:FRBNF(\d{8})|https?://[\w.]+\.bnf\.fr/ark:/12148/cb(\d{8}))[0-9A-Za-z]?")
...
def bnf_association_identifier(value):
    ...
    """
    if match := BNF_NUMBER.fullmatch(value or ""):
