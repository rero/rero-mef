...
def chunks(iterable, size=100000):
    """Split a generator in chunks"""
    iterator = iter(iterable)
    for first in iterator:
        yield chain([first], islice(iterator, size - 1))


def index_agency(agency, chunksize=100000, enqueue=False, verbose=False):
    """Index agency."""
    with current_app.app_context():
        agencies = current_app.config.get('AGENCIES')
        count = 0
        for chunk in chunks(agencies[agency].get_all_ids(), chunksize):
            # we have to make a list out of generator for Celery
            chunk = list(chunk)
            if verbose:
                count += len(chunk)
                click.echo(
                    'Start indexing task for {agency}: {count}'.format(
                        agency=agency,
                        count=count,
                    )
                )
            if enqueue:
                index.delay(agency, chunk)
            else:
                index(agency, chunk)
