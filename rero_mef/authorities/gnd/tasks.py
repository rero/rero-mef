...
def process_records_from_dates(from_date=None, until_date=None,
    ...
    """
    # data on GND Servers starts on 1900-01-01
    name = 'gnd'
    days_spann = 4
    lastrun = None
    url, metadata_prefix, lastrun, setspecs = get_info_by_oai_name(name)

    request = Sickle(url, iterator=MyOAIItemIterator)
    ...
    for spec in setspecs:
        params = {}
        params['metadataPrefix'] = metadata_prefix or "MARC21-xml"
        ...
        if spec:
            params['set'] = spec
        # we have to make small date requests
        # because GND limits the result size
        ...
        while my_from_date <= my_until_date:
            ...
            try:
                for record in request.ListRecords(**params):
                    ...
                    try:
                        ...
                        pid = rec.get('pid')
                        rec, action, mef_action = GndRecord.create_or_update(
                            rec,
                            dbcommit=dbcommit,
                            reindex=reindex,
                            test_md5=test_md5
            ...
            if verbose:
                click.echo(
                    ('OAI-GND: {from_d} .. +{days_spann}').format(
...
def gnd_get_record(id, dbcommit=False, reindex=False, test_md5=False,
    ...
    return get_record(id, name='gnd', transformation=Transformation,
                      record_class=GndRecord, access_token=access_token,
                      identifier=identifier, dbcommit=dbcommit,
                      reindex=reindex, test_md5=test_md5, verbose=verbose,
                      kwargs=kwargs)
